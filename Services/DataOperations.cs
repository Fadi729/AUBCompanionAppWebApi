    ...
    public static class DataOperations
    {
        ...
        public static async Task<bool> CourseGivenInSemester(this DbSet<Course> _dbSet, string crn, string semesterID)
        {
            return await _dbSet.AnyAsync(course =>
                course.Crn.ToString().Equals(crn) && course.SemesterId == semesterID);
        }

        public static async Task<CourseTakenBy?> GetCourseTakenByAsync(this DbSet<CourseTakenBy> _dbset, Guid userID,
            int crn, string semesterID)
        {
            return await _dbset.AsNoTrackingWithIdentityResolution().FirstOrDefaultAsync(coursetakenby =>
                coursetakenby.UserId == userID && coursetakenby.CCrn == crn && coursetakenby.SemesterId == semesterID);
        }
        ...
        public static async Task<Post?> GetPostAsync(this DbSet<Post> _dbSet, Guid postID)
        {
            return await _dbSet.AsNoTrackingWithIdentityResolution()
                .FirstOrDefaultAsync(post => post.Id.ToString().Equals(postID));
        }
        ...
        {
            return await _dbset.AsNoTrackingWithIdentityResolution()
                .FirstOrDefaultAsync(like => like.PostId.Equals(postID) && like.UserId.Equals(userID));
        }

        public static async Task<Following?> GetFollowingAsync(this DbSet<Following> _dbset, Guid userID,
            Guid followingID)
        {
            return await _dbset.AsNoTrackingWithIdentityResolution().FirstOrDefaultAsync(following =>
                following.UserId.Equals(userID) && following.IsFollowing.Equals(followingID));
        }
